...
export default class Coat extends Component {
  constructor(props) {
    super(props);
    this.state = {
      customdata: [],
  ...
  componentDidMount() {
    axios
      ...
      .then(responce => {
        ...
        this.setState({
          Coat: this.state.customdata.filter(
            pro => pro.name == "Blazers" || pro.name == "Two Pcs"
          )
        });
      })
        ...
        renderItem={({ item, index }) => (
          <View style={{ flex: 1, marginLeft: 10, marginTop: 10, width: 150 }}>
            <TouchableOpacity
              ...
              // this.props.navigation.navigate("CutomizeDetail",{CustomDetail:this.state.Coat})
            >
              <ImageBackground
